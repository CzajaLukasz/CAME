...
def makeCIFAR(num, list, teset_raw, teset_label_raw, args):
    ...
    for i in range(tesize):
        ...
        imsave(f'{args.metaset_dir}/dataset_default/new_{str(num).zfill(3)}/org' + str(i) + ".png", data)
        imsave(f'{args.metaset_dir}/dataset_default/new_{str(num).zfill(3)}/syn' + str(i) + ".png", new_data[i].astype(np.uint8))  # <fix>
...
if args.workers > 1:
    ...
    for num in range(num_sets):
        makeCIFAR(num, list, teset_raw, teset_label_raw, args)
