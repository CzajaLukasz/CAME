...
def loadMnist(mode, path):
    ...
    for entry in path_and_labels:
        ...
        img = imageio.imread(mnist_folder + path)  # scipy.misc.imread had been deprecated
...
def random_rotation_new(data):
    rot = (-1 + 2 * np.random.rand()) * 15  # Random rotation
    grid = getGrid([28, 28])
    grid = rotate_grid_2D(grid, rot)
    grid += 13.5
    data = linear_interpolation_2D(data, grid)
...
def random_rotation_90(data, degree):
    rot = degree  # Random rotation
    grid = getGrid([28, 28])
    grid = rotate_grid_2D(grid, rot)
    grid += 13.5
    data = linear_interpolation_2D(data, grid)
