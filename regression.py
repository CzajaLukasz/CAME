...
def main():
    ...
    with torch.no_grad():
        ...
        if args.metaset_numLim >= len(f_list): assert "Out of list index"

        for i in trange(0, len(f_list)):
            ...
            con_top1, cla_top1 = simclr.test(metaset_loader)
    ...
    np.save(f'{args.save_dir}/accuracy_cla{epoch}.npy', cla_acc)
    ...
    R2 = slr.score(np.array(con_acc.reshape(-1, 1)), np.array(cla_acc.reshape(-1, 1)))
